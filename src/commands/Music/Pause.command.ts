...
export default class Pause extends Command {
    ...
    async callback(client: BotClient, interaction: CommandInteraction): Promise<any> {
        ...
        if(player.paused) return interaction.reply(new ReplyEmbed().warn("ตอนนี้กำลังหยุดชั่วคราวอยู่น่ะ"));
        player.pause();
		await interaction.reply(new ReplyEmbed().success("ทำการหยุดชั่วคราวเรียบร้อยเเล้ว"));
    }
};
